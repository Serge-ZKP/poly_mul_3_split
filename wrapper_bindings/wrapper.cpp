...
unsigned char *ZZ_p_to_string(const ZZ_p *z, long *len) {
    ZZ zz = conv<ZZ>(*z);
    *len = NumBytes(zz);
    unsigned char* x = new unsigned char[*len + 1];
    BytesFromZZ(x, zz, *len);
    return x;
}

void ZZ_p_destroy(ZZ_p *x) {
    delete x;
}
...
void destroy_unsigned_char(unsigned char *x){
    delete x;
}
